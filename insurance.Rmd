...
rate (i.e., 1-specificity|probability of false detection) at various probability 
thresholds. Importantly, the area under the ROC curve (AUC) offers a measure of 
...
Tests of model validity for the transformation model are comparable to the reduced 
model and, to avoid repetition, are not reported here. Diagnostics for model
performance are included below and in section 4. 
...
There are a variety of methods to aid model selection, both quantitative and
qualitative. Here selection will proceed using a cross-section of diagnostics 
that aim for:
...
It's worth noting that diagnostics for the transformation model produced similar
results as the reduced model. Both are considered valid.

From the first table below (i.e., comparison of model results) it is clear that 
the transformed model is an appropriate choice. It produces the lowest AIC & BIC 
values and the lowest residual deviance. Additionally, the AUC for our 
transformed model is equivalent to the reduced and full models (.81) and greater 
than the null model (.50). It does, however, produce higher std. 
errors for transformed covariates. 

Other measures of performance are included in the classification metrics table 
below (reduced and transformation models). Note, however, that they are specific 
to the probability thresholds selected for each model. And are not considered
criterion for model selection.

Additional tests can be performed to confirm our model choice (e.g., likelihood 
ratio tests for nested models) but are not necessary. 
...
To assess how well the transformation model generalizes, we apply it to the 
test dataset and summarize the results. Interestingly, the AUC produced on the 
test set has increased (.82) relative to the training set - indicating that the
model generalizes well. 

Assume for a moment that an insurer seeks to minimize their risk of
claim payouts (relative to client payments). In this instance, they might 
elect to lower the probability threshold used to classify new clients. 

Below, our results for model performance reflect a cutoff of ~0.25. The model 
classifies 90% of our positives (target_flag = 1, crash) correctly, as compared 
to 49% of our negatives (target_flag = 0, no accidents).
...
We can now build a linear model to predict payout cost (target_amt) to a driver 
in the event of a crash. 

First, build a new a new training and test set (using random selection)
for this purpose.


distinction between avg payout over set of claims or an individual claim --- indv, see o vals for oldclm and clm_freq

common sense - bluebook, car_age, car_type, oldclaim/clm_freq = avg payout in past, predictor for future

value_clm = target_amt/bluebook -- use this to parse out payment for damages other than collision, filtered 282 obs

Note - there are no pairwise correlations -.5<>.5
...
**6.1 Characterize Response Variable: Target Amt**

The response variable (target_amt) is highly right skewed. This can be 
problematic when constructing a multiple regression model using ordinary 
least squares. 

options for analyzing skewed outcomes that are potentially heterodscedastic - robust statistics (e.g., weighted least squares) transformation of the response variable(log). The former are appropriate for analyses involving few covariates.Transformations can lead to interpretive issues for the response on the original scale.

Generalized linear models do not require normally distributed errors and homoscedasticity. Gaussian (variance unrelated to mean - homosced), gamma (pos skew, nonneg, variance proportional to square of the means), and inverse-Gaussian distributions (variance proportional to mean cubed) can be specified for continuous responses. 

For gamma family - link function can be specified with a log or an inverse transformation function (allows us to get expected value in original scale). Explore these two.

Insurance claim prediction:

McCullagh and Nelder, Generalized Linear Models. 2nd ed.(http://www.utstat.toronto.edu/~brunner/oldclass/2201s11/readings/glmbook.pdf) 
...
Included below is a full set of bivariate plots for numerical and factor
predictors and our response, 'target_amt'. 

Note that ~80% of the paid claims (target_amt) total less than $10,000. 
To assist visual inspection, this latter value will be used as ceiling
in the scatterplots below.

It is difficult to discern a linear relationship between numerical covariates
and 'target_amt'. In contrast, there does appear to be a linear relationship 
between levels of car_type and 'target_amt'. 


highest payouts - young cars, middle bb value, few to no past clms
...
Prior to modeling, we can visually assess potential covariate interactions as 
predictors of 'target_amt'. It appears that 'car_type' influences the 
relationship between each numerical covariate and 'target_amt'. In contrast,
there to not appear to be any interactions among numerical 
covariates in relation to 'target_amt'
...
```
**6.4 Linear GLMZ: Null Model**

Given that the response variable ('target_flag') consists of positive values
and is strongly right skewed, several options for model building are available. 
They include the following generalized linear forms:

- log transformed gaussian model (link = identity)
- gamma model (link = log or inverse)
- poisson model (link = log)

A gaussian model with a log transformed response may be difficult to interpret,
particularly if additional steps are required to address heteroscedasticity. A
gamma model is well suited to the modeling task and interpretation is straight-
forward. In this case, a log link is a good starting choice and is necessary
for a multiplicative model(i.e., interactions). A poisson is a possibility but
is restricted to integer values for target_amt. 

We will start with a null model with a log transformed response and assume that
it is valid (unlikely).
...
null_glm <- glm(log(target_amt) ~ 1, family='gaussian', data=linear_train)

summary(null_glm)

```
**6.5 Linear Model: Full w/ Main Effects**
...
```
**6.6 Linear Model: Reduced, w/ Interactions**

```{r}

gauss_interact <- lm(log(target_amt) ~ bluebook*car_type+car_age*car_type+avg_clm*car_type, family='gaussian', data=linear_train)
...
```
**6.7 linear model: Gaussian reduced and predictors transformed**
...
**6.8 GLMZ Model: Gamma (link=log), Reduced**
...
glm1<-glance(gamma_reduced)

```

**6.8.1 Diagnostics: Gamma Reduced**

constant variance perhaps
...
# check if selected distribution is appropriate

qqnorm(qresid(gamma_reduced)); qqline(qresid(gamma_reduced))
...
plot(density(linear_test$target_amt), ylim=c(0, .0008), main='Actual vs Fitted (red): Target_Amt')

lines(density(predict(gamma_reduced, type='response')), col='red')

```


**6.9 GLMZ Model: Gamma (link=log) w/ interactions**

Note log transform on predictors did not help
...
glm2<-glance(gamma_reduced_int)



```

**6.9.1 Diagnostics: GLMZ Model: Gamma (link=log) w/ interactions**
...
col_names<-c('Gamma-No Interactions', 'Gamma-Interactions')

glm_results<-rbind(glm1, glm2)
...
predictions<-predict(gamma_reduced_int,linear_train, type='response')

gamma_fit_plot<-cbind(linear_train, predictions)

ggplot(gamma_fit_plot, aes(x=target_amt, y=predictions))+
    geom_point()+
    theme_minimal()
...
In general,
the amount of payout is determined by such factors as:
