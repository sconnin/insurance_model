...
##4. Linear Model: tidymodels##

We can now build a linear model to predict payout cost (target_amt) to a driver 
in the event of a crash. In this instance, we will conduct our model building
process using tidymodels. 
...
linear_df<-df%>%
    dplyr::select(-home_val, -target_flag)%>%
    filter(target_amt > 0)
...
linear_split<-initial_split(linear_df, prop = 3/4, strata = target_amt)

linear_train<-training(linear_split)
linear_test<- testing(linear_split)

write_csv(linear_train, "linear_mod.csv")  # temp file for model development
View(linear_test)
```

**4.1 Characterize Response Variable: Target Amt**
...
numeric_df<-linear_test%>%
    dplyr::select(is.numeric)

xvar_names<-linear_test%>%
...
scatter_func <- function(x){
    ggplot(ceiling, aes_string(x=x, y='target_amt'))+ 
    geom_point(alpha=.1)+
    geom_smooth(method = "loess", se = FALSE, color='red')+
    theme_minimal()
}
...
plot(reduce(scatter_plots,`+`)) # reduce for single print

# create function for boxplots

factor_df<-linear_test%>%
    dplyr::select(is.factor)

target_amt<-linear_test$target_amt

factor_df<-cbind(target_amt, factor_df)

xvar_fact_names<-factor_df%>%
    dplyr::select(-target_amt)%>%
    names() # numerical covariates

xvar_fact_names<-purrr::set_names(xvar_fact_names)

box_fun = function(x) {
    ggplot(num_box, aes_string(x = x, y = 'target_amt') ) +
    geom_boxplot(aes(fill = target_flag, alpha = 0.4), outlier.color =
    'red', show.legend = FALSE)+
    scale_fill_viridis(discrete = TRUE, option = "E")+
    coord_flip()+
...
box_plots<-map(xvar_, ~box_fun(.x)) #creates a list of plots using purrr
...
To optimize model interpretability, it may be preferable
to build a generalized linear model based on either a gamma or inverse gaussian distribution.
