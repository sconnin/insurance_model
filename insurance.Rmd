...
![Project Workflow](/Users/seanc/Documents/Data_Projects/insurance_model/images/tidymodel.jpg)
...
```
# Data Processing and Holdout {.tabset}
...
## Count and Frequency of Target Variable
...
The ability of a binary classifier to discriminate class membership can be evaluated using a confusion (error) matrix as well as a receiver operating 
...
While the majority of covariates contain outliers, there is little evidence for influential outliers (excepting obs. 5101) in the training set 
...
back-transformation and must also adhere to the requirements of ordinary least squares. In instances where heteroscedasticity is apparent a log-normal model may be modified using weighted least squares (smaller datasets) or robust techniques. 
...
[McCullagh and Nelder, 1983](http://www.utstat.toronto.edu/~brunner/oldclass/2201s11/readings/glmbook.pdf) 
[Mohammad and McGovern, 2020](https://files.eric.ed.gov/fulltext/ED607605.pdf)
...
It's worth noting that ~80% of the paid claims are less than $10,000 each. And the highest payouts are associated with newer vehicles, few -to- no past claims, and mid-range bluebook values. It appears that payouts are proportionally less for
high-value vehicles.
...
Prior to modeling, we can visually assess potential covariate interactions as predictors of 'target_amt'. It appears that 'car_type' influences the 
relationship between each numerical covariate and 'target_amt'. In contrast, there do not appear to be any interactions among numerical covariates in relation to 'target_amt'
...
```
## Log-Normal Model: Full

Next we build a full log-normal model with main effects. Only 'bluebook' and 'car_type' are significant at an alpha = 0.5. The
adjusted error is only 0.03. 
...
[Dunn, Peter K. and Gorden K. Smyth, 2018](https://link.springer.com/book/10.1007/978-1-4419-0118-7)
[Balajari, 2013](http://civil.colorado.edu/~balajir/CVEN6833/lectures/GammaGLM-01.pdf)
